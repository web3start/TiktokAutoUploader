...
def subprocess_jsvmp(js, user_agent, url):
    try:
        ...
        if proc.returncode != 0:
            print(f"[-] Node.js signature generator failed with exit code {proc.returncode}")
            if stderr:
        ...
        if stderr:
            ...
            if stderr_text:
                print(f"[WARNING] Signature generator stderr: {stderr_text}")
        
        return stdout.decode('utf-8')
    except subprocess.TimeoutExpired:
        proc.kill()
        print("[-] Node.js signature generator timed out after 60 seconds")
        print("[-] This may indicate an issue with Playwright browser in headless mode")
        return None
    except FileNotFoundError:
        print("[-] Node.js not found. Please ensure Node.js is installed and in PATH")
        return None
    except Exception as e:
        print(f"[-] Unexpected error running signature generator: {str(e)}")
        return None
